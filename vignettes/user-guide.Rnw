...
%\VignetteDepends{knitr, photobiology, photobiologyWavebands, ggspectra, photobiologyInOut, ggplot2, ggmap, lubridate}
...
<<>>=
jaz.spct <- read_oo_jazirrad(file = "spectrum.JazIrrad")
jaz0.spct <- jaz.spct
...
We can see that the data have probelms. We get a warning because the data contains 
...
<<>>=
plot(read_oo_jazirrad(file = "spectrum.JazIrrad"))
@

As above but limiting the wavelength range plotted.

<<>>=
plot(read_oo_jazirrad(file = "spectrum.JazIrrad"), range = c(250,850))
@

Adding our custom ``adaptive'' smoothing.

<<>>=
plot(smooth_spct(read_oo_jazirrad(file = "spectrum.JazIrrad")), range = c(250,850))
@

Now a file from an Ocean Optics' Q6500? spectrometer.

<<>>=
plot(read_oo_sstxt(file = "spectrum.SSIrrad"))
@

Avantes' two column .csv files can also be imported.

<<>>=
plot(read_avaspec_csv(file = "spectrum-avaspec.csv"), 
...
<<>>=
plot(read_macam_dta(file = "spectrum.DTA"))
@
...
<<>>=
jaz1.spct <- read_oo_jazirrad(file = "spectrum.JazIrrad", date = NULL)
jaz1.spct
@
...
<<>>=
jaz.spct <- read_oo_jazirrad(file = "spectrum.JazIrrad", date = now())
jaz.spct
@

\section{Output from solar spectrum simulation models}
...
<<>>=
tuv.spct <- read_tuv_usrout(file = "usrout.txt", date = ymd("2014-03-21"))
...
<<>>=
tuv_nd.spct <- read_tuv_usrout(file = "usrout.txt")
tuv_nd.spct
@
...
Meateorological Institute (FMI) by Dr.\ Anders Lindfors and collaborators.

We can read an individual file into a \texttt{source\_spct} object while adding
some metadata.

<<>>=
z.spct <- read_fmi_cum("2014-08-21_cum.hel")
class_spct(z.spct)
...
<<>>=
z.mspct <- read_m_fmi_cum("2014-08-21_cum.hel")
class(z.mspct)
...
<<>>=
z.mspct <- read_m_fmi_cum(c("2014-08-21_cum.hel", 
                            "2014-08-22_cum.hel"))
class(z.mspct)
getWhenMeasured(z.mspct)
z.mspct
...
<<>>=
files <- system("ls ./*cum.hel", intern = TRUE)
