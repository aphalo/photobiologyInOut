...
A general way of exchanging data with other R packages or for use with base R functions is to create a matrix. 
...
Collections of spectra have a \texttt{spct.byrow} attribute and this is used as default, however, a value can by passed as argument to override this default.
...
These functions are not fully automatic, the user needs to provide the name of the variable to extract from each spectrum. If the wavelength values are not consistent among spectra, only those with the same values as the first spectrum in the colection are retained and the remaining ones dropped with a warning.
